...
**內容**：将 `gear_ops/api/*.py` 中的业务逻辑提取到服务层
...
#### 步骤 1.8：更新依赖
編輯 `platform/user_core/requirements.txt`，确保包含：
```
...
- `POST /api/gear/items` → 建立装备
- `PUT /api/gear/items/{id}` → 更新装备
- `DELETE /api/gear/items/{id}` → 刪除装备

---

### Phase 3：清理旧代码

#### 步骤 3.1：刪除 gear_ops 目录
```bash
# 备份（可选）
...
#### 步骤 3.2：更新文档
- 刪除 `docs/gear-ops/migration-plan.md`（针对独立服务的迁移计划）
...
# 3. 运行資料库迁移：alembic upgrade head
...
- [ ] `platform/frontend/ski-platform/.env.development` - 刪除 VITE_GEAR_API_URL
- [ ] `platform/frontend/ski-platform/.env.production` - 刪除 VITE_GEAR_API_URL
