...
2. **Given** 既有使用者需要新增「教練」身份，**When** 更新角色標記，**Then** 系統同時紀錄變更時間並通知依賴專案。

---

### 使用者故事 2 - 統一行為事件紀錄（優先度：P2）
...
- 核心事件清單與欄位定義紀錄於 `specs/shared/event_catalog.yaml`，新增或升級需先更新該型錄。
...
1. **Given** 使用者調整通知設定，**When** 偏好被儲存，**Then** 系統紀錄變更時間與來源並觸發事件給訂閱者。
...
- **FR-001**：系統必須能建立、更新、查詢、停用單一 `User ID` 主檔，並紀錄操作歷史。
...
- **FR-008**：系統必須紀錄資料變更審計資訊（操作者、時間、欄位、舊值、新值）。
