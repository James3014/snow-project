# SkiDIY Platform Project Registry
...
| gear-ops | 裝備檢查與買賣 | 管理裝備檢查記錄、交易流程與推播提醒 | 裝備檢查、買賣紀錄、推播設定 | user-core | 要標記用途（自用/教學）以支援未來推薦；與 snowbuddy-matching 共用通知機制 |
...
- **事件記錄格式**：所有子專案寫入的行為事件需遵守 `user-core` 定義的統一 schema（時間戳、事件類型、來源專案、payload）。
