...
def create_event_for_user(event: behavior_event_schema.BehaviorEventCreate, db: Session = Depends(db.get_db)):
    try:
        return behavior_event_service.create_event(db=db, event=event)
